...
for k = 2:(kMax+1)
    ...
    if (mod(k,10) - 2) == 0
        ...
        if k == 2
            [Uc,fval,exitflag,output,final_pop] = ga(@(Ucon) FitnessFunc_v4(Ucon,Np,Nc,Nb,A,B,C,y_lo,y_hi,S,R,T,E,beta_levels,economic_loss_by_level,cumulative_economic_loss,k,beta,Npop,dT,Xc),3,[],[],[],[],[1 1 1],[5 5 5],[],[1 2 3]);
        else
            [Uc,fval,exitflag,output,final_pop] = ga(@(Ucon) FitnessFunc_v4(Ucon,Np,Nc,Nb,A,B,C,y_lo,y_hi,S,R,T,E,beta_levels,economic_loss_by_level,cumulative_economic_loss,k,beta,Npop,dT,Xc),3,[],[],[],[],[1 1 1],[5 5 5],[],[1 2 3]);
        end
