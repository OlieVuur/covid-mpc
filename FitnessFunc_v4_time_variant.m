function obj = FitnessFunc_v4_time_variant(Ucon,Np,Nc,Nb,alpha,gamma,delta,lambda,kappa,B,C,E,economic_loss,cumulative_eco_loss,yLo,yHi,S,R,T,Betas,k,beta,Npop,dT,X0)

    % Initialize control states and outputs
    Y_eval = zeros(size(C,1),Np-1);
    X_eval = zeros(size(C,1),Np-1);
    X_eval(:,1) = X0;
    Y_eval(:,1) = X0;
    ...
    while (i < Np)
        ...
        A = [-alpha   0      0             0         0 0 0;
           ...
           0     0     delta  (-kappa(k+i-1) - lambda(k+i-1)) 0 0 0;
           0      0      0          lambda(k+i-1)       0 0 0;
           0      0      0          kappa(k+i-1)        0 0 0;
           alpha  0      0             0            0 0 0];
